...
void main() {
    ...
    float dc = linearizeDepth(texture2D(depths, tc0).x);
    vec4 dd;
    dd.x = linearizeDepth(texture2D(depths, tc1).x) + linearizeDepth(texture2D(depths, tc2).x);
    dd.y = linearizeDepth(texture2D(depths, tc3).x) + linearizeDepth(texture2D(depths, tc4).x);
    dd.z = linearizeDepth(texture2D(depths, tc5).x) + linearizeDepth(texture2D(depths, tc6).x);
    dd.w = linearizeDepth(texture2D(depths, tc7).x) + linearizeDepth(texture2D(depths, tc8).x);
    ...
    // smoothed color
    vec2 offset = tc0 * (1 - w);
    vec4 s0 = texture2D(diffuse, offset + tc1 * w);
